...
public class BillGenerate {
	...
	public BillGenerate(List<OrderItem> selectedItems, String customerName, long customerNumber2, String tableNumber) {
	...
	public void generateBill() {
		...
		int option = JOptionPane.showOptionDialog(null, "Select an option to proceed:", "Save/Print Bill",
				JOptionPane.DEFAULT_OPTION, JOptionPane.INFORMATION_MESSAGE, null,
				new String[] { "Print Bill", "Cancel" }, "Print Bill");
	...
	private double calculateAndDisplayBill() {
		...
		billDetails.append("<h2>Hotel Sawariya And Family Resort - Invoice</h2>");
		billDetails.append("<p><strong>Table Number:</strong> ").append(tableNumber).append("</p>");
		...
		for (OrderItem item : selectedItems) {
			...
			billDetails.append("<tr>").append("<td>").append(item.getName()).append("</td>").append("<td>")
					.append(item.getQuantity()).append("</td>").append("<td>₹").append(item.getPrice()).append("</td>")
					.append("<td>₹").append(itemTotal).append("</td>").append("</tr>");
		}
		...
		billDetails.append("<p><strong>GST (5%):</strong> ₹").append(gstAmount).append("</p>");
	...
	private void saveBillToDatabase(double totalAmount) {
		try (var conn = databaseCode.getConnection(); var stmt = conn.createStatement()) {
			String insertBillQuery = "INSERT INTO bills (order_id, customer_name, customer_number, total_amount, bill_date) "
					+ "VALUES (NULL, '" + customerName + "','" + customerNumber + "', " + totalAmount
					+ ", CURRENT_TIMESTAMP)";
